# Case Study: Inventory Analytics For PUMA and ADIDAS

[📄 View Full Project PDF](./inventory_analytics.pdf)

## 📌 1. The Bottleneck (Problem)
[The correlation betwen inventory and financial performance, the core inventory imbalances or financial drags PUMA and ADIDAS were facing in this study.]

## 🛠️ 2. Methodology & Tools
* **Analysis Framework:** Global Competition Positioning System
* **Core Metrics:** Inventory Turnover Ratio, Days Inventory Outstanding (DIO), Working Capital Impact

## 📊 3. Core Analysis & Findings
* **Diagnosed specific inventory problems** altering financial performance patterns across both sportswear brands.
* **Provided structural recommendations** to optimize stock levels and free up cash flow trapped in excess warehouse inventory.
